...
parser.add_argument("--global_pruning", help="global pruning?", type=str, default="False")
args_cl = parser.parse_args()

assert args_cl.eparallel[0] == "_"; assert args_cl.adaptive in ["True", "False"]; assert args_cl.global_pruning in ["True", "False"]; 
...
if args_cl.no_samples != -1:
    results_folder.folders = sample_result_folders(results_folder=results_folder, no_samples=args_cl.no_samples)
no_exps, start, end = get_folder_idx(orig_folder_length=len(results_folder.folders), k=k, T=T)
pruning_ratios = MAGNITUDE_PRUNING_RATIOS
results_part_string = get_results_part_string(args_cl.eparallel, args_cl.no_samples)
    
_ = magnitude_pruning(results_folder, pruning_ratios, args_cl.pruned_layers_str, args_cl.x_type, write_results=True, results_part_string=results_part_string, start=start, end=end, adaptive=args_cl.adaptive == "True",  global_pruning=args_cl.global_pruning == "True")